...
async def fetch_trackid_cooccurrence(
    *,
    ...
    Returns rows ordered by setCount desc (then pairUrl asc for stable ties),
    shaped as TrackMeta. Separate per-source helpers so each source can evolve
    its TTL / source label independently — see commit 8314220 for the rationale.
    """
...
async def upsert_trackid_cooccurrence_batch(
    *,
    ...
    EXCLUDED.setCount replaces (not sums) on conflict so re-scrapes don't
    double-count. gen_random_uuid is used for the id since Prisma's cuid() is
...
async def upsert_candidate_features_batch(rows: list[dict]) -> None:
    ...
    trackid cooccurrence writer above).
    """
    if not rows:
        return
    pool = await _get_pool()
