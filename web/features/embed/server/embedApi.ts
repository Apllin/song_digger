...
const EmbedQuerySchema = z.object({
  ...
  artistKey: z.string().optional(),
  titleKey: z.string().optional(),
});

export const embedApi = new Hono<AppEnv>().get("/embed", zValidator("query", EmbedQuerySchema), async (c) => {
  const { title, artist, artistKey, titleKey } = c.req.valid("query");

  const cached = await lookupEmbedCache(artist, title, { artistKey, titleKey }).catch(() => null);
